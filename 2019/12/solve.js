...
const parseMoonsFromInput = (rows) =>
  rows.map((row) => {
...
const calculateMoonEnergy = (moon) =>
  calculateMoonKineticEnergy(moon) * calculateMoonPotentialEnergy(moon);

const calculateTotalMoonEnergy = (moons) =>
  moons.reduce((sum, moon) => {
    sum += calculateMoonEnergy(moon);
    return sum;
  }, 0);

const calculatePositions = (moons, velocities) =>
  moons.map((moon, index) =>
    ['x', 'y', 'z'].reduce((position, axis) => {
...
const isSameState = (a, b) =>
  a.every((state, index) =>
    ['x', 'y', 'z'].every(
      (axis) =>
        state.position[axis] === b[index].position[axis] &&
...
const solve1 = () => {
  getRows().then((rows) => {
    const totalEnergy = calculateTotalMoonEnergy(
      calculateNewState(parseMoonsFromInput(rows), 1000),
    );
